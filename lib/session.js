...
function Location(loc) {
	debug('Creating location: %j', loc);
	var location = loc.LocationInfo[0];
	
	this.locationID = location.LocationID[0];
	this.devices = _.map(location.Thermostats[0].ThermostatInfo, function(device) { return new Device(device); });
	this.timeZone = location.TimeZone[0];
...
Session.prototype.getLocations = function() {
	...
	GetLocations(locationsParams).then(function(body) {
		parseString(body[1], function(err, result){
		     if(err === null || err === '') {
 				var locations = _.map(result['soap:Envelope']['soap:Body'][0].GetLocationsResponse[0].GetLocationsResult[0].Locations, function(loc) { return new Location(loc); });
